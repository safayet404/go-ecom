...
func (h *Handler) RegisterRoutes(mux *http.ServeMux, manager *middleware.Manager) {
	...
	mux.Handle("PATCH /products/{productId}", manager.With(http.HandlerFunc(h.UpdateProduct), middleware.AuthJwt))
	mux.Handle("DELETE /products/{productId}", manager.With(http.HandlerFunc(h.DeleteProduct), middleware.AuthJwt))
